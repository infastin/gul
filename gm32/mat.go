...
func (m *Mat) Det() float32 {
	...
	default:
		...
		for i := 0; i < cp.M; i++ {
			...
			for j := i + 1; j < cp.M; j++ {
				a1 := Abs(cp.Data[i+j*cp.N])
				a2 := Abs(cp.Data[i+k*cp.N])
				if a1 > a2 {
					k = j
				}
			}

			if Abs(cp.Data[i+k*cp.N]) < EPS {
				return 0
			}
